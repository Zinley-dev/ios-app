//
//  StitchControlVC.swift
//  Stitchbox
//
//  Created by Khoi Nguyen on 7/2/23.
//
...
class StitchControlVC: UIViewController, UICollectionViewDelegateFlowLayout, UIAdaptivePresentationControllerDelegate {
...
extension StitchControlVC: ASCollectionDataSource {
...
extension StitchControlVC {
    ...
    func collectionNode(_ collectionNode: ASCollectionNode, didDeselectItemAt indexPath: IndexPath) {
        if let node = collectionNode.nodeForItem(at: indexPath) as? OwnerPostSearchNode {
            ...
            selected_itemList = selected_itemList.filter { $0.id != item.id }
        }
    }
