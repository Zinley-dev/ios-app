...
class MutedMemberVC: UIViewController, UINavigationControllerDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(MutedMemberVC.unmutedUser), name: (NSNotification.Name(rawValue: "MutedMember-unmuteUser")), object: nil)
    ...
    func getUsers() {
        ...
        for member in filteredMembers {
            ...
            if self.channel?.getMember(addedUser.userId)?.isMuted == true {
    ...
    func setupBackButton() {
        ...
        backButton.setTitle("     Muted Members", for: .normal)
    ...
    @objc func onClickBack(_ sender: AnyObject) {
        if let navigationController = self.navigationController {
            NotificationCenter.default.removeObserver(self, name: (NSNotification.Name(rawValue: "MutedMember-unmuteUser")), object: nil)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if userList.isEmpty {
            tableView.setEmptyMessage("No muted member")
        } else {
            tableView.restore()
        }
    ...
    @objc func actionButtonPressed(sender: UIButton) {
        ...
        let UnmutedUserModView = UnmutedUserModView()
        UnmutedUserModView.modalPresentationStyle = .custom
        UnmutedUserModView.transitioningDelegate = self
    
        //setting frame
        global_presetingRate = Double(0.15)
        global_cornerRadius = 30
        
        self.present(UnmutedUserModView, animated: true, completion: nil)
 
    }
    
    
    @objc func unmutedUser() {
